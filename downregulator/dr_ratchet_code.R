...
#             A = matrix(c(-1,1,0,-2), nrow = 2, ncol = 2 ), 
#             B = matrix(c(1,1), nrow = 2, ncol = 1), 
#             C = matrix(c(1,0), nrow=1, ncol=2)
#        )
sys0 <- list(
             name = "a random system",
             A = matrix(rnorm(4,0,1), nrow = 2, ncol = 2 ), 
             B = matrix(rnorm(2,0,1), nrow = 2, ncol = 1), 
             C = matrix(rnorm(2,0,1), nrow=1, ncol=2)
        )
...
ransys_costdel <- function(sys0, std, m)
{
    ...
    for (i in 1:n)
    {
        ...
        dist_list[i] <- tryCatch(D(sysd$A, sysd$B, sysd$C, optimal_h=sys0$optimal_h), error=function(e) NaN)
    }
...
dim_cost <- data.frame(n=nvals,
                       mean_distance=tapply(cost_rsys$mean_distance, 
                                            factor(cost_rsys$n, levels=nvals), mean, na.rm=TRUE),
...
dim_cost$percent_fit <- sapply(nvals, function (nval) {
                                   ...
                                   return( sum(sub_cost$min_distance <= 0.2, na.rm=TRUE) 
